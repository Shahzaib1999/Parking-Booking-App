...
class Details extends Component {
    ...
    delete1(doc, id) {
        ...
        db.collection("parkingArea1").doc(doc).collection('Booking').doc(id).delete().then(function () {
    ...
    delete2(doc, id) {
        ...
        db.collection("parkingArea2").doc(doc).collection('Booking').doc(id).delete().then(function () {
    ...
    getData() {
        ...
        db.collection('parkingArea1').orderBy('id', 'asc').get().then(res => {
            res.forEach(doc => {
                ...
                db.collection('parkingArea1').doc(doc.id).collection('Booking').where("uid", "==", firebase.auth().currentUser.uid).get().then(querySnapshot => {
                    if (querySnapshot.size > 0) {
                        ...
                        querySnapshot.docs.map(data => {
                            ...
                            jumbotron.setAttribute("class", "jumbotron border border-primary");
                            btn.setAttribute("class", "btn btn-primary float-right ml-2");;
                            btn.setAttribute('id', 'cancel');
                            btn.addEventListener('click', () => this.delete1(doc.id, data.id))
        ...
        db.collection('parkingArea2').orderBy('id', 'asc').get().then(res => {
            res.forEach(doc => {
                ...
                db.collection('parkingArea2').doc(doc.id).collection('Booking').where("uid", "==", firebase.auth().currentUser.uid).get().then(querySnapshot => {
                    if (querySnapshot.size > 0) {
                        ...
                        querySnapshot.docs.map(data => {
                            ...
                            btn.setAttribute('id', 'cancel');
                            btn.addEventListener('click', () => this.delete2(doc.id, data.id))

                            btn.innerHTML = "Cancel Booking";
                            h3.innerHTML = "Parking Area 2";
                            h41.innerHTML = "Slot: " + doc.data().id;
                            h42.innerHTML = "Date: " + data.data().day;
                            h43.innerHTML = "Start Time: " + starthour + ":" + startmin;
                            h44.innerHTML = "End Time: " + endhour + ":" + endmin;
        ...
        db.collection('parkingArea3').orderBy('id', 'asc').get().then(res => {
            res.forEach(doc => {
                ...
                db.collection('parkingArea3').doc(doc.id).collection('Booking').where("uid", "==", firebase.auth().currentUser.uid).get().then(querySnapshot => {
                    if (querySnapshot.size > 0) {
                        ...
                        querySnapshot.docs.map(data => {
                            ...
                            btn.setAttribute("class", "btn btn-primary float-right");;
                            btn.setAttribute('id', 'cancel');
                            btn.addEventListener('click', () => this.delete3(doc.id, data.id))

                            btn.innerHTML = "Cancel Booking";
                            h3.innerHTML = "Parking Area 3";
                            h41.innerHTML = "Slot: " + doc.data().id;
                            h42.innerHTML = "Date: " + data.data().day;
                            h43.innerHTML = "Start Time: " + starthour + ":" + startmin;
                            h44.innerHTML = "End Time: " + endhour + ":" + endmin;
    ...
    render() {
        return <div className="container mt-4" id="data">
            <center>
                <input type="button" className="btn btn-info float-center mt-5" value="Show Bookings" onClick={this.getData} />
            </center>
        </div>;
    }
}
